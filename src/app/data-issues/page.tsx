    ...
    <AppShell active="Data issues" eyebrow="Portfolio integrity" issueCount={data.issues.length} title="Data issues">
      <section className="page-card">
        {data.issues.length === 0 ? (
          <div className="empty-state"><CircleCheck aria-hidden="true" /><h2>No unresolved data issues</h2><p>Every open position has an ISIN, a current market price, and a valid CHF conversion rate. Invalid transaction sequences would also appear here.</p><PriceRefreshButton /><Link className="primary-button" href="/">Return to overview</Link></div>
        ) : (
